...
exports.setBookmarkIds =
	exports.getBookmarkIds =
	exports.getArtifact =
	exports.getArtifactById =
	exports.getArtifactsByAssociation =
	exports.getArtifactsByBookmarks =
	exports.getArtifactsByIds =
	exports.getArtifactsByUser =
	exports.getArtifactsByPool =
		void 0;
...
async function getArtifactsByPool(args) {
	...
	return getArtifactsResponseObject(gqlResponse, types_1.CursorEnum.GQL, args.reduxCursor);
}
...
async function getArtifactsByIds(args) {
	...
	if (
		args.cursor &&
		args.cursor !== config_1.CURSORS.p1 &&
		args.cursor !== config_1.CURSORS.end &&
		!(0, utils_1.checkGqlCursor)(args.cursor)
	) {
		cursor = args.cursor;
	}
	...
	return getArtifactsResponseObject(artifacts, types_1.CursorEnum.Search, args.reduxCursor);
}
...
async function getArtifactsByBookmarks(args) {
	...
	return getArtifactsResponseObject(artifacts, types_1.CursorEnum.GQL, args.reduxCursor);
}
...
async function getArtifactsByAssociation(associationId, sequence) {
	...
	if (associationId) {
		...
		for (let i = 0; i < gqlArtifacts.data.length; i++) {
			const associationSequence = (0, utils_1.getTagValue)(
				gqlArtifacts.data[i].node.tags,
				config_1.TAGS.keys.associationSequence
			);
...
async function getArtifact(artifact) {
	let pool = await (0, pools_1.getPoolById)((0, utils_1.getTagValue)(artifact.node.tags, config_1.TAGS.keys.poolId));
	try {
...
function getArtifactsResponseObject(gqlResponse, cursorObject, reduxCursor) {
	// TODO: reimplement in site
	// let cursorState: any;
	// if (reduxCursor) {
