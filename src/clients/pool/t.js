	...
	switch (action.input.function) {
		case 'contribute': {
			...
			if (totalContributions == BigInt(0)) {
				...
				// calculate new mints
				const mintedTokens =
					(Number((BigInt(1000000000000) * contribution) / totalContributions) / 1000000000000) * Number(totalSupply);
		...
		case 'increaseUsedFunds': {
			if (state.owner !== caller) {
				throw new ContractError('Only the owner can increase used funds.');
			}

			if (state.usedFunds) {
				state.usedFunds = state.usedFunds + action.input.data;
			}
